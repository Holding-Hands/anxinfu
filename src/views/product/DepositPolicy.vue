...
// 切换启用/禁用状态
const handleToggleStatus = async (row: DepositPolicyItem) => {
  const isEnabled = row.status === '启用'
  const action = isEnabled ? '禁用' : '启用'

  try {
    await ElMessageBox.confirm(`确定要${action}该押金政策吗？`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
...
const handleBatchEnable = async () => {
  if (selectedIds.value.length === 0) {
    ElMessage.warning('请先选择要启用的押金政策')
    return
  }

  try {
    await ElMessageBox.confirm(
      `确定要启用选中的 ${selectedIds.value.length} 个押金政策吗？`,
      '提示',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
...
const handleBatchDisable = async () => {
  if (selectedIds.value.length === 0) {
    ElMessage.warning('请先选择要禁用的押金政策')
    return
  }

  try {
    await ElMessageBox.confirm(
      `确定要禁用选中的 ${selectedIds.value.length} 个押金政策吗？`,
      '提示',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
