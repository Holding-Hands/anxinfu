...
// 切换启用/禁用状态
const handleToggleStatus = async (row: TrafficPolicyItem) => {
  const isEnabled = row.status === 1
  const action = isEnabled ? '禁用' : '启用'

  try {
    await ElMessageBox.confirm(`确定要${action}该流量费政策吗？`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
    ...
    const url = isEnabled
      ? '/index/product_sim/setdisable.html'
      : '/index/product_sim/setenable.html'
    const formData = new URLSearchParams()
    formData.append('ids', String(row.id))
...
const handleBatchEnable = async () => {
  if (selectedIds.value.length === 0) {
    ElMessage.warning('请先选择要启用的流量费政策')
    return
  }

  try {
    await ElMessageBox.confirm(
      `确定要启用选中的 ${selectedIds.value.length} 个流量费政策吗？`,
      '提示',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
...
const handleBatchDisable = async () => {
  if (selectedIds.value.length === 0) {
    ElMessage.warning('请先选择要禁用的流量费政策')
    return
  }

  try {
    await ElMessageBox.confirm(
      `确定要禁用选中的 ${selectedIds.value.length} 个流量费政策吗？`,
      '提示',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
