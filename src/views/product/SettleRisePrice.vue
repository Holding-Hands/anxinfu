...
// 切换启用/禁用状态
const handleToggleStatus = async (row: SettleRisePriceItem) => {
  const isEnabled = row.status === 1
  const action = isEnabled ? '禁用' : '启用'

  try {
    await ElMessageBox.confirm(`确定要${action}该上涨结算价吗？`, '提示', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
    ...
    const url = isEnabled
      ? '/index/settle_rise/setdisable.html'
      : '/index/settle_rise/setenable.html'
    const formData = new URLSearchParams()
    formData.append('ids', String(row.id))
...
const handleBatchEnable = async () => {
  if (selectedIds.value.length === 0) {
    ElMessage.warning('请先选择要启用的上涨结算价')
    return
  }

  try {
    await ElMessageBox.confirm(
      `确定要启用选中的 ${selectedIds.value.length} 个上涨结算价吗？`,
      '提示',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
...
const handleBatchDisable = async () => {
  if (selectedIds.value.length === 0) {
    ElMessage.warning('请先选择要禁用的上涨结算价')
    return
  }

  try {
    await ElMessageBox.confirm(
      `确定要禁用选中的 ${selectedIds.value.length} 个上涨结算价吗？`,
      '提示',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
