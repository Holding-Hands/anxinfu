  ...
  <div class="page-container">
    ...
    <el-card shadow="never">
      ...
      >
        ...
        <!-- 回调数量 -->
        <el-table-column prop="nums" label="回调数量" min-width="100" align="center" />
        ...
        <!-- 退货用户 -->
        <el-table-column prop="user_name" label="退货用户" min-width="150" align="center" />

        <!-- 接收用户 -->
        <el-table-column prop="recevie_user" label="接收用户" min-width="120" align="center" />

        <!-- 退货时间 -->
        <el-table-column prop="create_time" label="退货时间" min-width="180" align="center" />
    ...
    >
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="回调数量">{{ currentRow?.nums }}</el-descriptions-item>
        <el-descriptions-item label="退货用户">{{ currentRow?.user_name }}</el-descriptions-item>
        <el-descriptions-item label="接收用户">{{ currentRow?.recevie_user }}</el-descriptions-item>
        <el-descriptions-item label="操作员">{{ currentRow?.operator }}</el-descriptions-item>
        <el-descriptions-item label="退货时间">{{ currentRow?.create_time }}</el-descriptions-item>
        <el-descriptions-item label="订单号" :span="2">
          {{ currentRow?.order_code }}
        </el-descriptions-item>
        <el-descriptions-item label="SN串" :span="2">
          <div style="word-break: break-all">{{ currentRow?.sn_str }}</div>
        </el-descriptions-item>
      </el-descriptions>

      <!-- 退货用户详细信息 -->
      <el-divider content-position="left">退货用户详细信息</el-divider>
      <el-descriptions v-if="currentRow?.from" :column="2" border>
        ...
        <el-descriptions-item label="用户类型">
          {{
            currentRow.from.user_type === 1
              ? '代理'
              : currentRow.from.user_type === 2
                ? '商户'
                : '未知'
          }}
        </el-descriptions-item>
      ...
      <el-empty v-else description="无退货用户详细信息" :image-size="100" />
...
import { getTuihuoLogListApi, type TuihuoLogParams, type TuihuoLogItem } from '@/api/tuihuo'
import { PRODUCT_OPTIONS } from '@/constants'

// 查询参数
const queryParams = reactive<TuihuoLogParams>({
  page: 1,
  limit: 15,
  platform_id: '',
  sn_str: '',
...
// 表格数据
const tableData = ref<TuihuoLogItem[]>([])
const total = ref(0)
const loading = ref(false)

// 详情对话框
const detailDialogVisible = ref(false)
const currentRow = ref<TuihuoLogItem | null>(null)

// 获取列表数据
const getList = async () => {
  loading.value = true
  try {
    const res = await getTuihuoLogListApi(queryParams)
    console.log('回调记录响应:', res)
...
// 查看
const handleView = (row: TuihuoLogItem) => {
