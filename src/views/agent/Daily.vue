    ...
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          <span class="card-title">每天业绩列表</span>
        </div>
      </template>
      ...
      >
        ...
        <el-table-column prop="user_name" label="所属盟友" min-width="200" align="center">
          <template #default="{ row }">
        ...
        <el-table-column prop="day" label="日期" min-width="120" align="center">
          <template #default="{ row }">
            {{ formatDay(row.day) }}
          </template>
        </el-table-column>
...
import { getDayListApi, type DayListParams, type DayListItem } from '@/api/user'
import { SYS_TYPE_OPTIONS, PRODUCT_OPTIONS, CARD_TYPE_OPTIONS } from '@/constants'

// 查询参数
const queryParams = reactive<DayListParams>({
  page: 1,
  limit: 15,
  sys_type: '',
  product_id: 0,
...
// 表格数据
const tableData = ref<DayListItem[]>([])
const total = ref(0)
...
const getList = async () => {
  loading.value = true
  try {
    const res = await getDayListApi(queryParams)
    console.log('每天业绩列表响应:', res)
    console.log('数据示例:', res.data?.[0])

    // 根据实际返回的数据结构处理
    const rawData = res.data || []
    // 过滤掉无效数据
    tableData.value = rawData.filter((item: DayListItem) => item && item.id)
...
const formatDay = (day: number | string): string => {
  ...
  // 格式: 20251106 -> 2025-11-06
  if (dayStr.length === 8 && !dayStr.includes('-')) {
    return `${dayStr.substring(0, 4)}-${dayStr.substring(4, 6)}-${dayStr.substring(6, 8)}`
  }
  // 如果已经是 2025-11-06 格式，直接返回
  return dayStr
}

// 合计行
const getSummaries = (param: { columns: { property?: string }[]; data: DayListItem[] }) => {
  const { columns } = param
  const sums: string[] = []
